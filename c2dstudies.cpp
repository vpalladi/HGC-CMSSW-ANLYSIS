...
//        new histoContainer<TH2D> ( hhC2DcentreVsNcells         , TString("centre(????)"),   TString("N cells"),    TString("colz") ),
    ...
    for( int ievt=0; ievt<nEvt; ievt++ ){
        ...
        if( detector.areTCpresent() ){
            ...
            /* get the vector */
            vector<HGCTC*> tcs = detector.getTCall();
            ...
            /* tower data printout*/
            if( verboselevel >= 4 ){
                cout << " MAIN >> Tower Data into the vector " << towerData.size() << endl;
                for(unsigned int ihgcroc=0; ihgcroc<towerData.size(); ihgcroc++ ) {
        ...
        if( detector.areC2Dpresent() ) {
            ...
            vector<HGCC2D*> C2Ds = detector.getC2Dall();

            unsigned nC2D = C2Ds.size();
            
            if(verboselevel >= 1)
                cout << " MAIN >> C2D analysis, " << endl
                     << " there are " << nC2D << " C2Ds in the event" << endl;
            ...
            for(unsigned iclu=0; iclu<nC2D; iclu++){
                ...
                if( verboselevel >= 2 ) {
                    ...
                    if( iclu%100 == 0 )
                        cout << " MAIN >> " << iclu << " C2D processed (" << percent << "%)" << endl;
                } 
